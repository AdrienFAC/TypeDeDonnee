...
let rec exec = function 
	...
	|(x,Cons::c,Val(y)::st) -> exec(PairV(x,y),c,st)
	...
	|(BoolV(true),Branch(t,e)::c,Val(x)::st) -> exec(x,t,Cod(c)::st) (* cas then *)

	|(BoolV(false),Branch(t,e)::c,Val(x)::st) -> exec(x,t,Cod(c)::st) (* cas else *)
